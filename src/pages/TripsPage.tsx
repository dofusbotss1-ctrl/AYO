...
import { Filter, Grid, List, Search, SlidersHorizontal, X, MapPin, Calendar } from 'lucide-react';
import { useApp } from '../context/AppContext';
import TripCard from '../components/ProductCard';

const TripsPage: React.FC = () => {
  const { state, dispatch } = useApp();
  const [searchParams] = useSearchParams();
  const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid');
  const [sortBy, setSortBy] = useState<'destination' | 'price' | 'newest'>('newest');
  ...
  useEffect(() => {
    console.log('TripsPage useEffect - searchParams changed:', {
      categoryFilter,
      searchFilter,
      currentLocalSearch: localSearch,
      stateSearch: state.searchQuery
    });
    ...
    // Si pas de paramètre de recherche mais qu'on a une recherche locale, la vider
  ...
  // Synchroniser la recherche locale avec l'état global
  useEffect(() => {
  ...
  const filteredTrips = state.trips
    .filter(trip => {
      const matchesCategory = !state.selectedCategory || trip.category === state.selectedCategory;
  ...
  const handleSearch = () => {
    ...
    if (localSearch.trim()) {
      newUrl.searchParams.set('search', localSearch);
    } else {
  ...
  const handleSearchChange = (value: string) => {
    console.log('TripsPage handleSearchChange:', value);
  ...
  const clearFilters = () => {
    ...
    // Rediriger vers la page trips sans paramètres
        ...
        <div className="container mx-auto px-4">
          <div className="text-center text-white space-y-6">
            ...
            <p className="text-xl text-gray-200 max-w-2xl mx-auto">
              Découvrez {filteredTrips.length} figurine{filteredTrips.length > 1 ? 's' : ''} 
              {state.selectedCategory ? ` dans la catégorie ${getCategoryName(state.selectedCategory)}` : ' exceptionnelles'}
            </p>
            ...
            <div className="max-w-2xl mx-auto">
              <form onSubmit={(e) => { e.preventDefault(); handleSearch(); }} className="relative">
                <input
                  ...
                  className="w-full px-6 py-4 pl-12 pr-20 rounded-full text-gray-900 bg-white/95 backdrop-blur-sm focus:outline-none focus:ring-4 focus:ring-amber-200/50 text-lg"
                />
                <Search className="absolute left-4 top-1/2 transform -translate-y-1/2 w-6 h-6 text-gray-400" />
                <button
                  type="submit"
                  onClick={handleSearch}
                  className="absolute right-2 top-1/2 transform -translate-y-1/2 bg-amber-600 text-white px-4 py-2 rounded-full hover:bg-amber-700 transition-colors font-medium text-sm"
                >
                  Rechercher
                </button>
              </form>
          ...
          <div className="lg:w-80">
            <div className="bg-white rounded-2xl shadow-lg p-6 sticky top-24">
              ...
              <div className={`space-y-8 ${showFilters ? 'block' : 'hidden lg:block'}`}>
                {/* Categories */}
                <div>
                  <h4 className="font-semibold text-gray-900 mb-4 text-lg">Types de Voyage</h4>
                ...
                {/* Price Range */}
                <div>
                  <h4 className="font-semibold text-gray-900 mb-4 text-lg">Budget (DH)</h4>
                  <div className="space-y-4">
                    <div className="px-4">
                      <input
                        ...
                        style={{
                          background: `linear-gradient(to right, #d97706 0%, #d97706 ${(priceRange[1]/20000)*100}%, #e5e7eb ${(priceRange[1]/20000)*100}%, #e5e7eb 100%)`
                        }}
                      />
                    </div>
          ...
          <div className="flex-1">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 mb-8">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="flex items-center space-x-4">
                  ...
                  <select
                    value={sortBy}
                    onChange={(e) => setSortBy(e.target.value as 'destination' | 'price' | 'newest')}
            ...
            {filteredTrips.length > 0 ? (
              ...
              <div className="bg-white rounded-2xl shadow-lg p-16 text-center">
                ...
                {(localSearch || state.searchQuery) ? (
                  <>
                    <h3 className="text-2xl font-bold text-gray-900 mb-4">
                      Aucune figurine trouvée pour "{state.searchQuery || localSearch}"
                    </h3>
                    <p className="text-gray-600 mb-8 text-lg">
                      Désolé, nous n'avons pas de figurine correspondant à votre recherche.<br/>
                      Essayez avec d'autres mots-clés ou explorez nos autres figurines.
                    </p>
                  </>
                ) : (
                  <>
                ...
                >
                  {(state.searchQuery || localSearch) ? 'Effacer la recherche' : 'Voir toutes les figurines'}
                </button>
              </div>
            )}
          </div>
